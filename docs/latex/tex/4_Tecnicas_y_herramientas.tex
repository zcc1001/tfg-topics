...
\textbf{SCRUM}~\cite{agile_alliance} de forma adaptada al contexto de un
proyecto académico individual. Esta metodología permitió una gestión flexible y
adaptativa del proyecto. El trabajo se organizó en \textit{sprints} de dos
semanas, donde se definieron objetivos claros y se realizaron revisiones
periódicas para evaluar el progreso. Frente a metodologías estrictamente
...
fácilmente con GitHub, permite crear \textit{sprints}, épicas y tableros
Kanban.

Para seguir el flujo de trabajo de SCRUM y facilitar el proceso de desarrollo,
en este caso el tutor adoptó los roles de product owner y SCRUM master.
...
llevar un control de versiones eficiente utilizando Git~\cite{GitReference}.
...
elección se basa en la amplia gama de bibliotecas y \textit{frameworks} para el
procesamiento de datos y modelado de temas disponibles.
\subsubsection{Bibliotecas y frameworks}

El núcleo del proyecto se apoya en técnicas de NLP aplicadas al análisis de
documentos. Para el procesamiento de datos y modelado de temas se utilizaron
diversas bibliotecas de Python, entre las que destacan:
...
\begin{itemize}
    \item \textbf{LDA}~\cite{LdaTopicModelingb}: Implementación del modelo Latent Dirichlet Allocation para descubrir temas en grandes colecciones de documentos.
...
desarrollado un interfaz web utilizando la librería
...
\textbf{Docker Compose} como tecnología de contenedorización. Docker permite
encapsular la aplicación junto con todas sus dependencias en entornos
