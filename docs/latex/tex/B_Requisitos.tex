...
\begin{description}
	\item[\textbf{Metadatos de Gestión Académica:}] Información estructurada del contexto docente del \acrshort{tfg}, procedente del proceso de gestión académica de la asignatura. Incluye variables institucionales como curso lectivo, fechas de asignación/presentación, calificación final, tutores y estudiantes.
	\item[\textbf{Artefactos del Repositorio de Software:}] Información técnica y textual desarrollada durante la ejecución del \acrshort{tfg} y alojada en repositorios públicos de GitHub. Incluye la memoria en \LaTeX{}, archivos \textit{README}, histórico de \textit{issues} y \textit{abstracts}.
...
\begin{itemize}
	\item \textbf{Extracción de datos:} Integrar los metadatos de gestión académica y extraer los artefactos del repositorio de software (memorias \acrshort{tfg} en \LaTeX{}, \textit{readmes}, \textit{issues} y \textit{abstracts}) desde repositorios públicos de GitHub.
...
\begin{itemize}
	\item \textbf{RF-1. Extracción de datos:} El sistema debe descargar los artefactos del repositorio de software a partir de un listado de orígenes parametrizado, comunicándose con la \acrshort{api} de GitHub, e integrarlos con los metadatos de gestión académica disponibles.
...
\begin{description}
	\item[\textbf{Administrador / Desarrollador (Humano, Primario Backend):}] Perfil con conocimientos técnicos encargado de la configuración del sistema, la puesta en marcha de los entornos y la ejecución desatendida del \gls{pipeline} de datos en sus etapas de ingesta y procesamiento desde la CLI.
	...
	\item[\textbf{API de GitHub (Sistema Externo, Secundario):}] Interfaz de programación provista por GitHub que actúa como proveedor de los artefactos del repositorio de software. Es consumida por el \gls{pipeline} para descargar de forma automatizada memorias de \acrshort{tfg}, \textit{readmes}, \textit{issues} y \textit{abstracts} del repositorio semilla.
\end{description}
...
a cada uno de estos \acrshortpl{cu}, integrando los correspondientes casos de
prueba para cada funcionalidad.

% Caso de Uso 1
\tablaSmallSinColores{CU-1: Ejecutar \gls{pipeline} de ingesta.}
{p{0.21\textwidth} p{0.71\textwidth}}
{cu_1}
{
	\textbf{CU-1}                 & \textbf{Ejecutar \gls{pipeline} de ingesta}                                                                                \\
}
{
	\textbf{Versión}              & 1.0                                                                                                                  \\
	\textbf{Autor}                & Alumno                                                                                                               \\
	...
	\textbf{Casos de prueba}      & \textbf{Prueba 1 - Ingesta exitosa:} Verificar que el pipeline lee el \acrshort{csv} semilla y descarga todos los artefactos de GitHub generando archivos Parquet poblados. \newline \textbf{Prueba 2 - Error de Red/Token:} Validar que si el token es inválido o no hay red, el sistema captura el error y registra los avisos en el archivo de log sin detener la ejecución. \\
}
...
}
{
	...
	\textbf{Importancia}          & Alta                                                                                                            \\
	\textbf{Casos de prueba}      & \textbf{Prueba 1 - Entrenamiento exitoso:} Comprobar que tras leer los archivos Parquet de ingesta, el preprocesamiento \acrshort{nlp} limpia el texto, se entrenan los cuatro modelos y se guardan las métricas. \newline \textbf{Prueba 2 - Límite de memoria:} Validar el comportamiento del sistema cuando el volumen de textos del corpus es muy elevado, asegurando que no haya bloqueos o desbordamientos de memoria descontrolados. \\
}
...
}
{
	...
	\begin{enumerate}
		\def\labelenumi{\arabic{enumi}.}
		\tightlist
		\item El usuario accede a la pestaña de análisis de modelo en el frontend.
	...
	\textbf{Excepciones}          & Datos no disponibles si el modelo no fue entrenado (el frontend muestra advertencia).            \\
	\textbf{Importancia}          & Alta                                                                                                      \\
	\textbf{Casos de prueba}      & \textbf{Prueba 1 - Exploración correcta:} Verificar que al seleccionar un modelo y fuente en el menú desplegable, la interfaz carga interactivamente los gráficos de distribución y palabras clave de forma inmediata. \newline \textbf{Prueba 2 - Datos no disponibles:} Confirmar que si se intenta cargar un modelo no entrenado, la interfaz web muestra un mensaje de advertencia amigable en lugar de colapsar. \\
}
...
}
{
	...
	\textbf{Casos de prueba}      & \textbf{Prueba 1 - Visualización del ranking:} Validar que el panel comparativo muestra la tabla y los gráficos de rendimiento con las métricas de coherencia y diversidad de todos los modelos entrenados. \newline \textbf{Prueba 2 - Comparación con un solo modelo:} Verificar el comportamiento de la interfaz cuando solo hay un modelo entrenado, garantizando que el sistema indique la falta de modelos para realizar una comparación. \\
}
...
}
{
	...
	\begin{enumerate}
		\def\labelenumi{\arabic{enumi}.}
		\tightlist
		\item El usuario accede a la pestaña de análisis académico en el frontend.
	...
	\textbf{Excepciones}          & Ninguna.                                                                                                             \\
	\textbf{Importancia}          & Alta                                                                                                                 \\
	\textbf{Casos de prueba}      & \textbf{Prueba 1 - Filtrado dinámico:} Comprobar que los filtros por tutor, nota y rango de años en la vista académica actualizan la lista de proyectos y los gráficos de evolución. \newline \textbf{Prueba 2 - Búsqueda por texto libre:} Validar que la barra de búsqueda filtra correctamente los proyectos que contienen los términos clave en sus títulos o resúmenes. \\
}
